...
const expectedBenchmarks=JSON.parse(await fs.readFile(path.join(root,'docs/data/benchmarks.json'),'utf8'));
const expectedAiGovernance=expectedDecisionTools.find(item=>item.id==='ai-governance-security');
const expectedAiCapability=expectedBenchmarks.find(item=>item.id==='ai-capability');
if(!expectedAiGovernance)throw new Error('expected ai-governance-security decision tool is missing');
if(!expectedAiCapability)throw new Error('expected ai-capability benchmark is missing');
...
const checks=[
 ...
 ['intelligence/ai-capability.html',html=>html.includes('AI Capability')&&(expectedAiCapability.headline_eligible?!html.includes('noindex,follow'):html.includes('<meta name="robots" content="noindex,follow">'))],
