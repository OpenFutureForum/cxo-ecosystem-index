...
test('canonical sources and graph references are deterministic',()=>{
 ...
 assert.equal(graph.sourced_relationship_count,29);
 assert.equal(graph.derived_reciprocal_count,29);
 assert.equal(graph.relationships.filter(edge=>edge.derived_reciprocal).length,29);
});
...
test('governance exports are published',async()=>{
 ...
 assert.equal(graph.dataset_version,'0.9.6');
