...
test('cross-CXO guides are original structured resources with primary sources',async()=>{const guides=[...await readJSON('data/guides/cfo.json'),...await readJSON('data/guides/cxo.json')];assert.equal(guides.length,7);for(const g of guides){assert.ok(g.sections.length>=4);assert.ok(g.sources.length>=1);assert.ok(!JSON.stringify(g).toLowerCase().includes('efutures'))}});
